...
# Archetype codes are now DB-driven — no longer hardcoded here.
# The stage1_extract function's fallback still uses these for safety.
...
async def stage1_extract(request: Stage1Request) -> Stage1Response:
    ...
    if isinstance(raw_archetypes, list):
        for code in raw_archetypes:
            code_str = str(code)
            if code_str in _DEFAULT_ARCHETYPE_CODES and code_str not in seen:
...
async def stage3_vagueness_check(
    ...
    # Build Q/A block from the authoritative questions list (not just dict keys)
    # This ensures the AI sees the exact questions even if the client keys differ slightly
...
def _build_stage5_prompt(request: Stage5Request) -> str:
    ...
    budget_line = (
        ...
        else "\nESTIMATED BUDGET: Not provided — derive estimates from scope.\n"
    )
...
def _validate_stage5_response(raw: dict) -> Stage5Response:
    ...
    # Domain codes are now DB-driven — accept any non-empty string.
    # NestJS validates codes against the DB before sending to synthesis;
    domains = [
        {"domain_code": d["domain_code"], "required_depth": d.get("required_depth", "SURFACE")}
        for d in raw.get("required_domains_json", [])
        if isinstance(d, dict)
        and d.get("domain_code")
    ...
    milestones = [
        {
            ...
            "estimated_cost_vnd":     int(m.get("estimated_cost_vnd", 0)),      # E-2
            "estimated_duration_days": int(m.get("estimated_duration_days", 0)), # E-2
        }
    ...
    artifact_a = {
        ...
        # Accept any non-empty string archetype code — DB is now the authority
        "archetype":       str(raw_a.get("archetype", "1")) if raw_a.get("archetype") else "1",
...
async def milestone_chat(request: MilestoneChatRequest) -> MilestoneChatResponse:
    """
    Context-aware milestone assistant. The project spec is injected into the
    system prompt; the full conversation history arrives in request.conversation_history
    (loaded from DB by NestJS, so FastAPI stays stateless).
    """
    system_template = load_prompt("milestone_chat")
    system = (
        system_template
        .replace("{artifact_a}",
                 _json.dumps(request.artifact_a, ensure_ascii=False, indent=2))
        .replace("{milestone_framework}",
                 _json.dumps(request.milestone_framework, ensure_ascii=False, indent=2))
        .replace("{budget_context}", request.budget_context)
    )

    # conversation_history already contains the new user message at the end
    # (NestJS appends it before calling)
    ...
    # Extract optional ```edit_suggestion``` fence
    suggested_edit = None
    fence_match = _re.search(
        r"```edit_suggestion\s*(\{.*?\})\s*```",
        raw_reply,
        _re.DOTALL,
    )
    if fence_match:
        try:
            ...
            pass
        raw_reply = _re.sub(
            r"```edit_suggestion.*?```", "", raw_reply, flags=_re.DOTALL
        ).strip()
