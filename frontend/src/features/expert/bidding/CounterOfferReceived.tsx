...
export default function CounterOfferReceived() {
  ...
  return (
    <div className="mx-auto w-full max-w-[900px] space-y-6">
      ...
      {mutationError ? <div className="flex gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700"><AlertTriangle size={18} />{(mutationError as any)?.response?.data?.message?.message || (mutationError as any)?.response?.data?.message || 'Offer action failed.'}</div> : null}
