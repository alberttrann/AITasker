              ...
              {artifactB.schemas?.map((url, i) => {
                ...
                return (
                  <div key={`schema-${i}`} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-lg shadow-sm hover:border-emerald-300 transition-colors">
                    ...
                    <div
                      id={`scroll-expert-artifact-schema-${i}`}
                      role="region"
                      aria-label={`Schema ${i + 1}. Scroll horizontally to view the full value.`}
                      tabIndex={0}
                      className="min-w-0 flex-1 overflow-x-auto overscroll-x-contain rounded pb-1 focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-1"
                    >
                      {isLink ? (
                        <a href={href} target="_blank" rel="noreferrer" className="block w-max cursor-pointer whitespace-nowrap text-sm font-mono text-blue-600 hover:text-blue-800 hover:underline">
                          {url}
                        </a>
                      ) : (
              ...
              {artifactB.contracts?.map((url, i) => {
                ...
                return (
                  <div key={`contract-${i}`} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-lg shadow-sm hover:border-emerald-300 transition-colors">
                    ...
                    <div
                      id={`scroll-expert-artifact-contract-${i}`}
                      role="region"
                    ...
                    >
                      {isLink ? (
                        <a href={href} target="_blank" rel="noreferrer" className="block w-max cursor-pointer whitespace-nowrap text-sm font-mono text-purple-600 hover:text-purple-800 hover:underline">
                          {url}
                        </a>
                      ) : (
                        <span className="block w-max whitespace-nowrap text-sm font-mono text-slate-700">{url}</span>
