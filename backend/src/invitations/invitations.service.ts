...
export class InvitationsService {
  ...
  async getProjectInvitations(projectId: string, ceoId: string) {
    ...
    if (project.clientId !== ceoId)
      throw new ForbiddenException('Only the CEO can view project invitations.');
  ...
  async retractInvitation(invitationId: string, ceoId: string) {
    ...
    if (invitation.ceoId !== ceoId)
      throw new ForbiddenException('Only the sender can retract this invitation.');
    if (invitation.status === 'ACCEPTED') {
      throw new UnprocessableEntityException(
        'Cannot retract an accepted invitation — the expert has already submitted a bid.',
      );
    }
    return this.prisma.invitation.update({
      where: { id: invitationId },
